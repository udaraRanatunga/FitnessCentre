...
public class Signup implements MouseListener{
	...
	private JComboBox sex,chart,title,pay;
	private String [] type ={"Select","MALE","FEMALE"};
	private String [] list ={"Select a Payment Type","Month","Year"};
...
public void setFrame(){
        n = new FITNESS();
        signupFrame = n.getFrame("Member Signup",600,750);
    ...
    public void setTitle(){  
        mainTitle = n.getLabel("Member Signup", 140, 5, 350, 80, 30);
	...
	JLabel payLabel = n.getLabel("Payment Type", 20, 550);
        signinCont.add(payLabel);
	...
	public void setFields(){
		...
		pay = n.getComboBox(140,550,list);
		signinCont.add(pay);
	...
	public void mouseClicked(MouseEvent event){
		...
		if(event.getSource() == submit){
			...
			dox.setPayment(String.valueOf(pay.getSelectedItem()));
			...
			if(dox.getFname().isEmpty()){
				...
				msg.warningMsg("Payment Type should be selected",signupFrame);
			}
			else{
	...
	public void clearAll(){
		...
		pay.setSelectedItem("Select a Payment Type");
